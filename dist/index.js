...
// src/core/classes/ForgeCommand.ts
var import_commander = require("commander");
var ForgeCommand = class extends import_commander.Command {
  /**
...
async function ExecuteSearch(normalizedType, preparedObjectName, extension, dev, debug, forceFetch) {
  if (debug) {
    console.log(`${import_chalk.default.yellow("[DEBUG]")} Starting the search.`);
  }
  ;
...
var Search = new import_commander2.Command("search").aliases(["s", "lookup"]).description("Search for a specific function, enum or event in BotForge's documentation.").argument("<type>", "The type of object to search for (or their shortcuts).").argument("<object>", "The object name to search for (case insensitive).").option("-e, --extension <extension>", "Specify an extension to limit the search scope.").option("-r, --raw", "Output the result as raw JSON instead of formatted text.").option("-d, --dev", "Perform your research on the development branch.").option("--debug", "Show debug information during the search process.").option("--fetch", "Fetch information using HTTP request and forces to cache the results.").action(async (type, object, options) => {
  ...
  try {
    ...
    if (options.debug) {
      console.log(`
${import_chalk.default.yellow("[DEBUG]")} Requesting (GET) 'https://github.com/tryforge/ForgeScript/blob/dev/metadata/${NormalizedType}s.json' and potentially storing them inside cache (if caching cooldown is over).`);
    }
    ;
...
var ConfigurationManager = class {
  static {
    this.ConfigurationPath = import_path2.default.join(FileSystem.GetHomePath(), ".forge", "config.json");
  }
  /**
...
var Logger = class _Logger {
  ...
  static log(level, ...args) {
    ...
    switch (level) {
      case 0 /* TRACE */:
        args.forEach((arg) => console.log(`${import_chalk2.default.gray(`[${prefix}]`)}] ${import_chalk2.default.gray(arg)}`));
        break;
      case 1 /* DEBUG */:
        args.forEach((arg) => console.log(`${import_chalk2.default.yellow(`[${prefix}]`)} ${import_chalk2.default.yellow(arg)}`));
        break;
      case 2 /* INFO */:
        args.forEach((arg) => console.log(`${import_chalk2.default.blue(`[${prefix}]`)} ${import_chalk2.default.blue(arg)}`));
        break;
      case 3 /* SUCCESS */:
        args.forEach((arg) => console.log(`${import_chalk2.default.green(`[${prefix}]`)} ${import_chalk2.default.green(arg)}`));
        break;
      case 4 /* WARN */:
        args.forEach((arg) => console.log(`${import_chalk2.default.yellow(`[${prefix}]`)} ${import_chalk2.default.yellow(arg)}`));
        break;
      case 5 /* ERROR */:
        args.forEach((arg) => console.log(`${import_chalk2.default.red(`[${prefix}]`)} ${import_chalk2.default.red(arg)}`));
        break;
      case 6 /* FATAL */:
        args.forEach((arg) => console.log(`${import_chalk2.default.red(`[${prefix}]`)} ${import_chalk2.default.red(arg)}`));
        break;
...
var ProgressManager = class {
  ...
  Start(message) {
    ...
    this.Spinner = (0, import_ora2.default)({ text: message }).start();
  }
  /**
